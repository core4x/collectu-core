...
class ProcessorModule(AbstractProcessorModule):
    ...
    class Configuration(models.ProcessorModule):
        ...
        """
        tag_key: str = field(
            metadata=dict(description="The tag key which shall be passed (if invert is false). "
    ...
    def _run(self, data: models.Data) -> models.Data:
        ...
        for key, value in data.tags.items():
            ...
            if not self.configuration.invert:
                if key.lower() == str(self._dyn(self.configuration.tag_key)).lower():
                    # If only the existence of the key matters.
                    if str(self._dyn(self.configuration.tag_value)) == '*':
                        tags = {key: value}
                    # If also the value has to match the user input.
                    elif str(value).lower() == str(self._dyn(self.configuration.tag_value)).lower():
                        tags = {key: value}
            else:
                if key.lower() != str(self._dyn(self.configuration.tag_key)).lower():
                    tags = {key: value}
                else:
                    if self.configuration.tag_value == '*':
                        pass
                    elif str(value).lower() != str(self._dyn(self.configuration.tag_value)).lower():
                        tags = {key: value}
        data.tags = tags
