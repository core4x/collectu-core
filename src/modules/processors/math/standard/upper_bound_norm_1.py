...
class ProcessorModule(AbstractProcessorModule):
    ...
    description: str = "This module applies the upper bound function on the given key values."
    """A short description."""
    ...
    def _run(self, data: models.Data) -> models.Data:
        ...
        width = self.configuration.upper - self.configuration.lower

        def upper_bound_norm(value):
            return (1/width)*value + math.log((width*10)**(-self.configuration.lower/width) + (width*10)**(-value/width), width*10)
        ...
        if field_key == "*":
            ...
            for key, value in data.fields.copy().items():
                ...
                if isinstance(value, float) or isinstance(value, int):
                    data.fields[key] = upper_bound_norm(value)
                elif isinstance(value, list):
                    temp_values = []
                    for single_value in value:
                        if isinstance(single_value, float) or isinstance(single_value, int):
                            temp_values.append(upper_bound_norm(single_value))
            ...
            # Check if it is of type float, int or list.
            if isinstance(value, float) or isinstance(value, int):
                data.fields[self.configuration.result_key_name] = upper_bound_norm(value)
            elif isinstance(value, list):
                temp_values = []
                for single_value in value:
                    if isinstance(single_value, float) or isinstance(single_value, int):
