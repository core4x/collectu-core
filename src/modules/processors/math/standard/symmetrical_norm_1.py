...
class ProcessorModule(AbstractProcessorModule):
    ...
    description: str = "This module applies the symmetrical norm function on the given key values."
    """A short description."""
    ...
    class Configuration(models.ProcessorModule):
        ...
        result_key_name: str = field(
            ...
            default="result")
        center: float = field(
            metadata=dict(description="The center.",
                          required=False,
                          dyamic=False),
            default=1.0)
        width: float = field(
            metadata=dict(description="The width.",
                          required=False,
                          dyamic=False),
            default=1.0)
    ...
    def _run(self, data: models.Data) -> models.Data:
        ...
        def symmetrical_norm(value):
            # abs(2 / wi * (x - ce)) * (1 - exp(-(2 * 2 / wi * (x - ce)) ** 2))
            return abs(2/self.configuration.width * (value-self.configuration.center)) * (1 - math.exp(-(2 * 2/self.configuration.width * (value-self.configuration.center))**2))
        ...
        if field_key == "*":
            ...
            for key, value in data.fields.copy().items():
                ...
                if isinstance(value, float) or isinstance(value, int):
                    data.fields[key] = symmetrical_norm(value)
                elif isinstance(value, list):
                    temp_values = []
                    for single_value in value:
                        if isinstance(single_value, float) or isinstance(single_value, int):
                            temp_values.append(symmetrical_norm(single_value))
            ...
            # Check if it is of type float, int or list.
            if isinstance(value, float) or isinstance(value, int):
                data.fields[self.configuration.result_key_name] = symmetrical_norm(value)
            elif isinstance(value, list):
                temp_values = []
                for single_value in value:
                    if isinstance(single_value, float) or isinstance(single_value, int):
