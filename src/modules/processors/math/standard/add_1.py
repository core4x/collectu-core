...
class ProcessorModule(AbstractProcessorModule):
    ...
    description: str = "This module adds a defined value to key and tag values."
    """A short description."""
    ...
    class Configuration(models.ProcessorModule):
        ...
        """
        factor: float = field(
            metadata=dict(description="The value to add.",
                          required=False,
                          dynamic=True),
            default=None)
        key: str = field(
            metadata=dict(description="The field or tag key whose value shall be added. "
                                      "Value has to be int or float. "
                                      "If '*' is given, every int and float value is added.",
                          required=False,
                          dynamic=True),
            default="*")
    ...
    def _run(self, data: models.Data) -> models.Data:
        ...
        factor = self._dyn(self.configuration.factor, data_type=["int", "float"])
        if key != "*":
            ...
            if data.fields.get(key, False):
                if isinstance(data.fields.get(key), (int, float)) and not isinstance(data.fields.get(key), bool):
                    data.fields[key] = data.fields.get(key) + factor
            if data.tags.get(key, False):
                if isinstance(data.tags.get(key), (int, float)) and not isinstance(data.tags.get(key), bool):
                    data.tags[key] = data.tags.get(key) + factor
        else:
            for key, value in data.fields.items():
                if isinstance(value, (int, float)) and not isinstance(value, bool):
                    data.fields[key] = value + factor
            for key, value in data.tags.items():
                if isinstance(value, (int, float)) and not isinstance(value, bool):
                    data.tags[key] = value + factor
        return data
